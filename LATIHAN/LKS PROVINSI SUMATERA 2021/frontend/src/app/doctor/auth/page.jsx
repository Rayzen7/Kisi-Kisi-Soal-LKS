...
export default function doctorSignIn () {
  ...
  const handleLogin = async(e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:8000/api/v1/auth/doctor/login', formData);
    ...
    <div>
      ...
      <div className="w-full h-auto mt-16 flex flex-col justify-center items-center gap-24">
        ...
        <div className="bg-white w-[550px] min-h-[250px] pb-10 border-[rgba(0,0,0,0.11)] border-[1px] rounded-md">
          <p className="bg-[rgba(0,0,0,.03)] px-5 py-3 font-poppins font-[450] text-[22px] border-[rgba(0,0,0,0.11)] border-b-[1px]">Login as Doctor</p>
          <div className="font-poppins flex justify-center flex-col items-end mx-[40px] gap-4 mt-6">
            <div className="flex items-center gap-4">
              <label className="font-[400] text-[14px]">Username</label>
              <input 
                type="text" 
                value={username}
          ...
          <p className="font-poppins font-[400] text-center text-[16px] mt-4">Not a Doctor? <Link href='/' className="hover:underline text-primary">Login here</Link></p>
        </div>
      </div>
      <ToastContainer/>
    </div>
