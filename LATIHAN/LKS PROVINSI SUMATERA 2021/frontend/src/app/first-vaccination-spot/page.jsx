    ...
    return (
        <div className="">
            ...
            <div className="font-poppins pt-28 px-32">
                <h1 className="font-[350] text-[48px]">First Vaccination</h1>
                <div className="mt-20 flex flex-col gap-10">
                    <p className="text-[#6c757d] font-[400] text-[22px]">List Vaccination Spots in {spotName}</p>
                    <table className="border-collapse">
                        {spot.map((spots) => (
                            <tbody key={spots.id}>
                                <tr className={`font-poppins ${spots.serve == 2 ? 'opacity-40' : ''}`}>
                                    <td className="bg-[#FAFAFA] py-10 px-6">
                                        <h1 onClick={spots.serve == 2 ? false : () => handleDetail(spots.id)} className={`text-primary text-[20px] font-[500] ${spots.serve == 2 ? 'cursor-default no-underline' : 'cursor-pointer hover:underline'}`}>{spots.name}</h1>
