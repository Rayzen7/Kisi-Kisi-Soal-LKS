...
export default function FirstVaccination() {
    ...
    useEffect(() => {
        const fetchData = async() => {
            try {
                ...
                const response = await axios.get(`http://localhost:8000/api/v1/doctor/first?token=${dataToken}`);
    ...
    const handleUpdate = (id) => {
        navigate.push(`/doctor/detail/first-vaccination/${id}`);
    }
    return (
        <div className="font-poppins">
            <h1 className="font-[350] text-[48px]">First Vaccination Society</h1>
